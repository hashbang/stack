...
.PHONY: shell
shell: tools .local/images/stack-shell
	docker run \
		-it \
		--env UID="$(shell id -u)" \
		--env GID="$(shell id -g)" \
...
.local/images/stack-shell: images/stack-shell/Dockerfile
	docker build -t "local/$(NAME)-shell" -f "$<" .
	mkdir -p .local/images && touch .local/images/stack-shell

.local/images/stack-build: images/stack-build/Dockerfile
	docker build -t "local/$(NAME)-build" -f "$<" .
	mkdir -p .local/images && touch .local/images/stack-build
...
.local/bin/k3s: .local/images/stack-build
...
.local/bin/kind: .local/images/stack-build
...
.local/bin/sops: .local/images/stack-build
...
.local/bin/ksops-exec: .local/images/stack-build
...
.local/bin/kubectl: .local/images/stack-build
...
.local/bin/terraform: .local/images/stack-build
